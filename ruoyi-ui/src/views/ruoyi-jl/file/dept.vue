...
<script>
import { privateListFile,listFile, getFile, delFile, addFile, updateFile,deptListFile,openFile,updateStatus } from "@/api/ruoyi-jl/file";
...
export default {
  ...
  methods: {
    ...
    openFile(row) {
      ...
      if(fileType == 1){
        this.$modal.confirm('确定公开"' + fileName + '"文件？').then(function() {
          return openFile(fileId,2);
        }).then(() => {
          this.getList();
          this.$modal.msgSuccess("公开成功");
        }).catch(() => {});
      }else{
        this.$modal.confirm('取消公开"' + fileName + '"文件？').then(function() {
          return openFile(fileId,1);
        }).then(() => {
          this.getList();
    ...
    updateStatus(row) {
      ...
      const fileName = row.fileName;
      if(status == 0){
        this.$modal.confirm('确定将"' + fileName + '"文件归档？').then(function() {
          return updateStatus(fileId,1);
        }).then(() => {
          this.getList();
          this.$modal.msgSuccess("归档成功");
        }).catch(() => {});
      }else{
        this.$modal.confirm('取消将"' + fileName + '"文件归档？').then(function() {
          return updateStatus(fileId,0);
        }).then(() => {
          this.getList();
          this.$modal.msgSuccess("取消成功");
        }).catch(() => {});
